...
def _load_replay(state: DaemonState) -> str | None:
    ...
    if result != rd.ResultCode.Succeeded and gpu is not None:
        _log.warning(
            "OpenCapture failed with forced GPU (%s vendor=%d id=%d), "
            "retrying with default GPU selection",
            gpu.name, gpu.vendor, gpu.deviceID,
        )
        opts_fallback = rd.ReplayOptions()
        result, controller = cap.OpenCapture(opts_fallback, None)
    if result != rd.ResultCode.Succeeded:
        cap.Shutdown()
