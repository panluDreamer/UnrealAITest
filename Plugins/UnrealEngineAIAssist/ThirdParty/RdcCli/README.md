...
**From source (local project, managed by uv)**

```bash
cd <plugin_dir>/rdc-cli        # e.g. Plugins/UnrealEngineAIAssist/rdc-cli
uv tool install .              # install as a uv-managed CLI tool
rdc doctor                     # verify (bundled renderdoc.pyd included, no setup-renderdoc needed)
```

This is the recommended method when rdc-cli lives inside a project (e.g., UnrealEngineAIAssist plugin).
The package ships with precompiled `renderdoc.pyd` for v1.21 and v1.43 (Python 3.12) — no build tools required.

To update after pulling source changes:
```bash
cd <plugin_dir>/rdc-cli
uv tool install . --force      # rebuild from local source
```

To uninstall:
```bash
uv tool uninstall rdc-cli
```

**From source (pixi, for development)**
